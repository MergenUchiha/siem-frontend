...
function AppContent() {
    ...
    // FIX 4: keep a Set of known log IDs to prevent WebSocket duplicates
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [logsResponse, incidentsResponse] = await Promise.all([
                // FIX 2: logsApi.getAll() now returns PaginatedLogs — use .data
                logsApi.getAll({ limit: 100 }),
                incidentsApi.getAll(),
            ]);

            // logsResponse is always PaginatedLogs now — no need for type gymnastics
            const logsData: Log[] = logsResponse.data ?? [];
            const incidentsData: Incident[] = Array.isArray(incidentsResponse)
                ? incidentsResponse
                : ((incidentsResponse as any)?.data ?? []);

            // FIX 4: rebuild the known-IDs set when we do a full reload
    ...
    useEffect(() => {
        ...
        const handleNewLog = (newLog: Log) => {
            // FIX 4: skip if we already have this log (deduplication)
            if (logIdsRef.current.has(newLog.id)) {
                console.log("Duplicate log ignored:", newLog.id);
                return;
            }
        ...
        const handleNewIncident = (newIncident: Incident) => {
            // Deduplicate incidents by id as well
            setIncidents((prev) => {
    ...
    const handleRefresh = useCallback(() => {
        loadData();
        addNotification(
            t.notificationTypes.dataRefreshed,
            t.notificationTypes.dataRefreshedMessage,
            "info",
        );
    }, [loadData, t]);
    ...
    const handleLogin = () => {
        ...
        addNotification(
            t.notificationTypes.welcome,
            `${t.notificationTypes.welcomeMessage} ${user.name}`,
            "info",
        );
    };
                ...
                <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                        ...
                        <p className="text-gray-400">
                            {t.common.loading} SIEM Platform...
                        </p>
                    </div>
                </div>
            );
