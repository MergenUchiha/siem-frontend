...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const user = userString
    ...
    : { name: "User", email: "user@siem.local", role: "analyst" };
      ...
      >
        ...
        <div className="flex items-start justify-between p-4 border-b border-cyan-500/20">
          <div className="flex items-start space-x-3 flex-1 min-w-0">
            {/* University Logo */}
            <div className="w-11 h-11 rounded-xl overflow-hidden border border-cyan-500/30 bg-white flex-shrink-0 flex items-center justify-center">
              <img
                src="/logo.jpeg"
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden text-gray-400 hover:text-white transition-colors p-1 hover:bg-gray-800 rounded flex-shrink-0"
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                  <Icon
                    className={`w-5 h-5 transition-transform group-hover:scale-110 ${isActive ? "text-cyan-400" : ""}`}
                  />
